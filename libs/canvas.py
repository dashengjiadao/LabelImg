...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        for shape in reversed([s for s in self.shapes if self.isVisible(s)]):
            ...
            shapeIndex,shapeDisValue = shape.nearestVertex(pos, self.epsilon)
            if shapeIndex!=-1 and shapeDisValue<minShapeDis:
                minShapeIndex=shapeIndex
                minShapeDis=shapeDisValue
                minShape=shape
            
        if  minShapeIndex is not None:
            # Look for a nearby vertex to highlight. If that fails,
            # check if we happen to be inside a shape.
            if self.selectedVertex():
                self.hShape.highlightClear()
            self.hVertex, self.hShape = minShapeIndex, minShape
            minShape.highlightVertex(minShapeIndex, minShape.MOVE_VERTEX)
            self.overrideCursor(CURSOR_POINT)
            self.setToolTip("点击或拖动")
