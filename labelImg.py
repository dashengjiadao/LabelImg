...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        delete = action(getStr('delBox'), self.deleteSelectedShape,
                        's', 'delete', getStr('delBoxDetail'), enabled=False)
    ...
    def loadFile(self, filePath=None):
        ...
        # Highlight the file item
            ...
            if unicodeFilePath and os.path.exists(unicodeFilePath):
                ...
                self.setWindowTitle(__appname__ + ' ' + filePath +  " 进度：" + str(self.mImgList.index(self.filePath)+1) +"/"+ str(len(self.mImgList)))
    ...
    def changeSavedirDialog(self, _value=False):
        ...
        dirpath = ustr(QFileDialog.getExistingDirectory(self,
                                                       '%s - 标签文件夹' % __appname__, path,  QFileDialog.ShowDirsOnly
                                                       | QFileDialog.DontResolveSymlinks))

        if dirpath is not None and len(dirpath) > 1:
            self.defaultSaveDir = dirpath

        self.statusBar().showMessage('%s . 标签文件将保存到 %s' %
                                     ('修改保存路径', self.defaultSaveDir))
        self.statusBar().show()

    def openAnnotationDialog(self, _value=False):
        if self.filePath is None:
            self.statusBar().showMessage('请先选择标签文件夹')
            self.statusBar().show()
            return
        ...
        if self.labelFileFormat == LabelFileFormat.PASCAL_VOC:
            filters = "打开标签文件 (%s)" % ' '.join(['*.xml'])
            filename = ustr(QFileDialog.getOpenFileName(self,'%s - 选择标签文件' % __appname__, path, filters))
            if filename:
    ...
    def openDirDialog(self, _value=False, dirpath=None, silent=False):
        ...
        if silent!=True :
            targetDirPath = ustr(QFileDialog.getExistingDirectory(self,
                                                         '%s - 打开图片库文件夹' % __appname__, defaultOpenDirPath,
        ...
        try:
            ...
            self.errorMessage(u'缺少classes.txt文件',
                                      (u"<p><b>%s</b></p>"
                                       u"<p>当前目录下缺少classes.txt文件.")
                                      % (e))
    ...
    def openFile(self, _value=False):
        ...
        filename = QFileDialog.getOpenFileName(self, '%s - 选择标签或图片文件' % __appname__, path, filters)
        if filename:
            if isinstance(filename, (tuple, list)):
                filename = filename[0]
            self.loadFile(filename)
